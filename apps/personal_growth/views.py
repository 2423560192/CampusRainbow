...
from .models import Plan, LearningResource, EmotionRecord, AutoTask, PomodoroSession, PomodoroRecord, AIGeneratedPlan
...
class EmotionAnalyzeView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return success_response(
                data={
                    "analysis_id": str(emotion_record.id),
                    "primary_emotion": emotion_record.emotion,
                    "emotion_scores": {
                        "happy": 0.05 if emotion_record.emotion != "happy" else 0.8,
                        "sad": 0.05 if emotion_record.emotion != "sad" else 0.8,
                        "angry": 0.05 if emotion_record.emotion != "angry" else 0.8,
                        "frustrated": 0.05 if emotion_record.emotion != "frustrated" else 0.8,
                        "tired": 0.8 if emotion_record.emotion == "疲倦" else 0.05,
                        "anxious": 0.8 if emotion_record.emotion == "焦虑" else 0.05
                    },
                    "recommendation": emotion_record.adjustment_suggestion,
                    "recommendation_type": self._get_recommendation_type(emotion_record.emotion),
                    "created_at": emotion_record.created_at.isoformat()
                },
                message="情绪分析成功",
                code=200
            )
    ...
    def _get_recommendation_type(self, emotion):
        """根据情绪获取推荐类型"""
        ...
        return recommendation_types.get(emotion, "break")
    ...
    def post(self, request, task_id):
        ...
        if serializer.is_valid():
            ...
            if plan.status in ['active', 'completed']:
                return error_response(message="任务已启动或已完成", code=409)
            ...
            # 计算任务持续时间（分钟）
            task_duration_minutes = plan.focus_time

            # 使用Celery调度任务在指定时间后完成
            complete_task_after_time.apply_async(
                args=[task_id],
                countdown=task_duration_minutes * 60  # 转换为秒
            )
            ...
            if action == 'pause':
                ...
                if plan.status == 'active' and plan.start_time:
                    ...
                    if plan.savings_amount:
                        # 将 completion_ratio 转换为 Decimal 类型
                        from decimal import Decimal
                        completion_ratio = min(Decimal(plan.completed_focus_time) / Decimal(plan.focus_time),
                                               Decimal('1.0'))
...
class AIPlanDetailView(APIView):
    ...
    def get(self, request, plan_id):
        """获取指定ID的AI生成计划详情"""
        try:
            plan = AIGeneratedPlan.objects.get(id=plan_id, user=request.user)
            return success_response(
                data={
                    "plan_id": str(plan.id),
    ...
    def delete(self, request, plan_id):
        ...
        try:
            plan = AIGeneratedPlan.objects.get(id=plan_id, user=request.user)
            plan_description = plan.goal_description  # 保存计划描述以便在响应中使用
            plan.delete()

            return success_response(
                data={
                    "plan_id": plan_id,
                    "message": f"AI学习计划 '{plan_description}' 已删除"
                },
                message="AI计划删除成功",
                code=200
            )
